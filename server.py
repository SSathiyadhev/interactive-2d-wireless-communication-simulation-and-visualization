...
Supports:
- Explicit TX-RX link pairing
- Independent link evaluations
- Material rotation masks
- FastAPI HTTP server
- WebSocket-based simulation streaming
"""
...
def safe_call(obj, *names, default=0.0):
    """
    Safely retrieve either an attribute or method from an object.

    Example:
        safe_call(tx, "carrier_frequency",
...
class SimulationRuntime:
    ...
    def _build(self):
        ...
        self.space.set_global_permittivity(
            8.8541878128e-12
        )
    ...
    def add_receiver(
        ...
        if target_tx_id in self.transmitters:
            self.rx_tx_mapping[rx_id] = target_tx_id
        else:
            self.rx_tx_mapping[rx_id] = 0

        self._rebuild_evaluators()

    # ========================================================
    # Set receiver-transmitter link
    # ========================================================

    def set_rx_link(self, rx_id, tx_id):

        if (
            rx_id in self.receivers
            and tx_id in self.transmitters
        ):
    ...
    def _rebuild_evaluators(self):
        ...
        for rid, rx in self.receivers.items():
            ...
            target_tx_id = self.rx_tx_mapping.get(
                rid,
                0,
            )

            tx = (
                self.transmitters.get(target_tx_id)
                or next(
                    iter(self.transmitters.values()),
                    None,
                )
            )
    ...
    def add_material(
        ...
        if (
            rel_perm is not None
            and hasattr(
                mat,
                "set_relative_permittivity",
            )
        ):

            mat.set_relative_permittivity(
                float(rel_perm)
            )
        ...
        if (
            angle_rad != 0.0
            and hasattr(self.space, "grid_x")
            and hasattr(self.space, "grid_y")
        ):
            ...
            try:
                ...
                print(
                    f"Rotational mask overlay note: "
                    f"{rot_err}",
                    flush=True,
                )
    ...
    def clear_materials(self):
        ...
        if hasattr(
            self.space,
            "set_global_conductivity",
        ):

            self.space.set_global_conductivity(
                0.0
            )
    ...
    def field_bytes(self):
        ...
        field_display = np.flipud(
            quantized.T
        )

        return field_display.astype(
            np.uint8
        ).tobytes()
    ...
    def status(self):
        ...
        for rid, r in self.receivers.items():
            ...
            ev = self.evaluators.get(
                rid
            )

            ber_val = (
                ev.get_bit_error_rate()
                if (
                    ev
                    and ev.get_bit_error_rate()
                    is not None
                )
                else 0.0
            )
            ...
            delay_val = (
                ev.get_estimated_total_delay_seconds()
                * 1e9
                if ev
                else 0.0
            )
        ...
        # ----------------------------------------------------
        # Observation point FFT
        # ----------------------------------------------------

        freqs, amps = (
            self.obs_point.compute_fft(
                window_type="hann"
            )
        )
        ...
        peak_f, peak_a = (
            self.obs_point.get_peak_frequency(
                window_type="hann"
            )
        )

        window_energy = safe_call(
            self.obs_point,
            "get_windowed_energy",
            default=0.0,
        )
        ...
        return {
            ...
            "bits_compared": (
                active_ev.get_total_bits_compared()
                if active_ev
                else 0
            ),

            "bit_errors": (
                active_ev.get_bit_errors()
                if active_ev
                else 0
            ),
            ...
            "ber": (
                active_ber
                if active_ber is not None
                else 0.0
            ),

            "est_delay_ns": (
                active_ev.get_estimated_total_delay_seconds()
                * 1e9
                if active_ev
                else 0.0
            ),
            ...
            "rx_raw": list(
                active_rx.get_received_values()
            ),

            "rx_bpf": list(
                active_rx.get_filtered_values()
            ),

            "rx_mixed": list(
                active_rx.get_mixed_values()
            ),

            "rx_matched": list(
                active_rx.get_baseband_values()
            ),
...
def handle_control_message(message: dict):
    ...
    try:
        ...
        # ----------------------------------------------------
            ...
            new_id = (
                max(
                    runtime.transmitters.keys()
                ) + 1
                if runtime.transmitters
                else 0
            )

            runtime.add_transmitter(
                new_id,
                float(
                    message.get(
                        "x",
        ...
        # ----------------------------------------------------
            ...
            new_id = (
                max(
                    runtime.receivers.keys()
                ) + 1
                if runtime.receivers
                else 0
            )

            runtime.add_receiver(
                new_id,
                float(
                    message.get(
                        "x",
        ...
        print(
            "Error handling control message "
            f"{message}: "
            f"{type(e).__name__}: {e}",
            flush=True,
        )
...
async def ws_sim(websocket: WebSocket):
    ...
    try:
        ...
        print(
            "Initial status sent successfully.",
            flush=True,
        )
        ...
        print(
            "Initial WebSocket status error: "
            f"{type(e).__name__}: {e}",
            flush=True,
        )

        # Close cleanly if initial status fails
        try:
    ...
    # --------------------------------------------------------
    # Receive frontend control messages
    # --------------------------------------------------------
